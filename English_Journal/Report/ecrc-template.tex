...
Quadrotors have also been controlled using optimal controllers, such as the Linear Quadratic Gaussian (LQG) \cite{7367782} and the Linear Quadratic Regulator (LQR) \cite{7064553_LQR}. These controllers minimize a quadratic criterion by considering both regulation performance and control effort to provide optimal feedback gains.
...
The following sections of this research paper provide a comprehensive analysis of the proposed control approach. Section \ref{sec:problem} defines the problem, while sections \ref{sec:modeling} and \ref{sec:estimation} offer detailed derivations of the dynamics model and the estimation problem for the quadrotor's experimental platform, respectively. Section \ref{sec:controller} explicates the proposed LQIG-DG controller architecture. The effectiveness of the controller is evaluated in section \ref{sec:results}, which is followed by the conclusion in section \ref{sec:conclusion}.
...
During its rotation, the quadrotor can adjust its pitch, roll, and yaw angles.
 An Inertial Measurement Unit (IMU) is used to measure the acceleration and angular velocities along the three orthogonal axes, which are affected by noise.
...
Figure \ref{fig:schematic} illustrates the quadrotor schematic, depicting four rotors with angular velocity $\Omega_r$ rotating around the $z_B$ axis in the body coordinate system. The rotational direction of Rotor 1 and Rotor 3 in the counterclockwise direction counteracts the yawing moment, whereas the clockwise rotation of Rotors 2 and 4 counteracts the yawing moment.
...
The dynamic model of the Quadrotor Platrorm is an essential component for control design. In this section, the quadrotor dynamic model is derived using the Newton-Euler method \cite{4399042, article_Bouabdallah}.
...
\noindent The variables $(p, q, r)$ represent the rotational velocities, while the variables $d_{\text{roll}}$, $d_{\text{pitch}}$, and $d_{\text{yaw}}$ denote the disturbances produced in $x_B$, $y_B$, and $z_B$, respectively. Additionally, $\mathrm{I}{\text{xx}}$, $\mathrm{I}{\text{yy}}$, and $\mathrm{I}{\text{zz}}$ are the principal moments of inertia, and $\mathrm{I}{\text{rotor}}$ is the rotor inertia about its axis. Euler angle rates are also determined from angular body rates:
% \begin{align}
...
The system's input and disturbance matrices, denoted as $\boldsymbol{\mathrm{B}}$ and $\boldsymbol{\mathrm{B_d}}$ respectively, with $\boldsymbol{\mathrm{d}}$ representing the disturbance, are part of the system's dynamics, represented by the matrix $\boldsymbol{\mathrm{A}}$.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[font=\small,thick]
		% Start block
		\node[draw, align=center,		minimum width=2.5cm,		minimum height=1cm, fill=blue!10] (block1) {\textbf{Experimental data} \\Blade rotational speed as input variables \\ current attitude as output};
			
		% Voltage and Current Measurement
		\node[draw, align=center,		below=of block1,		minimum width=3.5cm,		minimum height=1cm, fill=blue!10	] (block2) {\textbf{Data preprocessing} \\ Synchronization experim, vs simulated data.};
		...
		Define error cost function to minimize.};
		...
		\node[draw,		right=2cm of block4,		minimum height=1cm,		minimum width=2.5cm,		fill=red!10		] (block6) {Insufficient empirical data};
...
\section{Optimal Control Strategy Formulation for a Nonlinear System}\label{sec:controller}
...
\noindent The LQR-DG controller is augmented with an integral action to eliminate steady-state errors in desired following. The first stage of the controller design process involves developing the augmented state space representation of the linear quadrotor model to incorporate it into the overall control architecture. Later on, the LQR-DG controller design methodology is introduced to generate optimal control signals for the experimental quadrotor platform.
...
The dynamics model of the quadrotor, as expressed by Eq. \eqref{eq:linear}, is reformulated into an augmented state-space model that incorporates the roll ($\phi$), pitch ($\theta$), and yaw ($\psi$) angles. The augmented state-space model provides a comprehensive description of the quadrotor's dynamic behavior, enabling the design of effective control strategies. The ensuing state-space model that integrates the augmented state variables is mathematically represented as follows:
...
\begin{equation}\label{systemlqidg}
	...
	\end{split}
\end{equation}
$\boldsymbol{\mathrm{A_a}}$ and $\boldsymbol{\mathrm{B_a}}$, which are defined as follows:
...
\noindent In accordance with the principles of differential game theory, the LQIR-DG controller is a robust and optimal methodology. This controller involves two fundamental players, one of which is responsible for determining the optimal control command, while the other strives to generate the worst possible disturbance. The primary objective of the former player is to minimize a cost function, whereas the latter player seeks to maximize it. Thus, the quadratic cost function equation can be expressed using min-max operators, as follows:
\begin{equation}
...
In the aforementioned equation, the matrices $\boldsymbol{\mathrm{Q_i}}$ is a symmetric positive definite matrix, while ${{ R}}$ and ${{R_{d}}}$ are symmetric nonnegative definite matrices. The final time is denoted by $\mathrm{t_f}$. To address this issue, the connection between the general optimal problem and the LQIR problem is established \cite{LQDG}. As a result, the optimal control input for each control loop can be computed using the following formula:

\begin{align}
		{{u_i}}(t) &= -\boldsymbol{{\mathrm{K}}_{i}}(t) \boldsymbol{{\mathrm{x_{a_i}}}}(t)\\
	{{d_i}}(t) &=\boldsymbol{{\mathrm{K_{d_i}}}}(t)\boldsymbol{{\mathrm{x_{a_i}}}}(t)
\end{align}
where $\boldsymbol{{\mathrm{K_{d_i}}}}$  and $\boldsymbol{{\mathrm{K_i}}}$
are a time-varying gain, defined as follows:
\begin{align}
...
In the current investigation, the feedback control law is developed utilizing the steady-state values of the previously mentioned equations as $\mathrm{t_f}$ approaches infinity. The matrices $\boldsymbol{\mathrm{S_{a_i}}}$ and $\boldsymbol{\mathrm{S_{a_{d_i}}}}$ are determined by utilizing the matrices $\boldsymbol{\mathrm{B_{a_i}}}$ and $\boldsymbol{\mathrm{B_{a_{d_i}}}}$ along with the symmetric nonnegative definite matrices ${{ R}}$ and ${{R_{d}}}$. Specifically, $\boldsymbol{\mathrm{S_{a_i}}}$ is defined as $\boldsymbol{\mathrm{B_{a_i}}}R^{-1}\boldsymbol{\mathrm{B}^\mathrm{T}_{a_i}}$, while $\boldsymbol{\mathrm{S{a_{d_i}}}}$ is defined as $\boldsymbol{\mathrm{B}_{a_{d_i}}}R_{d}^{-1}\boldsymbol{\mathrm{B}^\mathrm{T}_{a_{d_i}}}$.
...
\noindent The current study presents an investigation of the efficacy of the LQIR-DG control strategy applied to the roll, pitch, and yaw control loops of an experimental quadrotor platform. To ensure optimal and robust performance, the controller parameters are tuned using the results of numerical simulations. The performance of the LQIR-DG controller is compared to that of other control strategies, including LQR, PID, and LQIR. The quadrotor's parameters are listed in Table \ref{tab:parameters}.

Furthermore, the weighting parameters of the LQIR-DG controller are provided in Table \ref{tab:control weight_new}.
...
\begin{table}[H]\label{tab:identification}
	\renewcommand{\arraystretch}{1.3}
...
\subsection{Assessment of LQIR-DG Control Strategy}
...
\begin{figure}[H]
	...
	\caption{Efficiency Evaluation of the LQIR-DG Controller for (a) Roll, (b) Pitch, (c) and Yaw Attitude Angles}
	\label{fig:result}
\end{figure}
...
\noindent This section investigates the capability of the LQIR-DG controller in rejecting input disturbances during regulation. To this end, a disturbance with an amplitude of 0.5 N is added to the input signal from 20 to 60 seconds. As depicted in Figure \ref{fig:disturbance}, the LQIR-DG controller demonstrates proficient coupling of the roll and screw channels, resulting in effective removal of the input disturbance. The performance of the controller is further evaluated by comparing the desired and actual roll angles in Figure \ref{fig:disturbance}\ref{sub@fig:disturbance_roll}, as well as the desired and actual pitch angles in Figure \ref{fig:disturbance}\ref{sub@fig:disturbance_pitch}. The results indicate that the LQIR-DG controller performs appropriately in mitigating the input disturbance.

\begin{figure}[H]
	\centering
	\subfloat[\label{fig:disturbance_roll}]{\includegraphics[width=.49\linewidth]{../Figure/implementation/disturbance/lqidg_roll}}\subfloat[\label{fig:disturbance_pitch}]{\includegraphics[width=.49\linewidth]{../Figure/implementation/disturbance/lqidg_pitch}
	}
	\hfil
...
Figure \ref{fig:weight}\ref{sub@fig:weight_roll} displays the performance of the LQIR-DG controller in terms of the comparison between the desired roll angle and the actual roll angle, while figure \ref{fig:weight}\ref{sub@fig:weight_pitch} presents a similar comparison for the desired and actual pitch angles. The implementation results indicate that the LQIR-DG controller performs efficiently in achieving the desired values, even with the uncertainty in the moments of inertia.
\begin{figure}[H]
	\centering
...
\begin{itemize}
	...
	\item LQIR-DG
	The LQIR-DG controller provides the best performance among the three controllers, as it can handle uncertainties, disturbances, and weight additions without requiring a re-design of the controller. The LQIR-DG controller provides accurate and robust control of the quadrotor system, with minimal errors and oscillations in the response.
\end{itemize}
...
\noindent This study has presented a novel approach for the implementation and evaluation of a linear quadratic with integral action controller based on differential game theory for level attitude control in an experimental quadrotor setup. The proposed controller design required an accurate model of the quadrotor in state-space form, which was linearized, and the model parameters were estimated. The LQIR-DG controller involved two players for each of the roll, pitch, and yaw channels. The first player optimized the control command for each channel based on a quadratic criterion, while the second player generated the worst disturbances.

The performance of the LQIR-DG controller was evaluated in level flight and compared to the LQR controller. The results showed the successful performance of the LQIR-DG method in level flight for attitude control, tracking square wave, disturbance rejection, and model uncertainty in the actual plant. Overall, this study has demonstrated the effectiveness and potential of the LQIR-DG method in quadrotor attitude control, which can be extended to other aerospace systems.
