...
in finding a vector $\boldsymbol{\mathrm{\Gamma}}$ that minimizes a sum of squares function [?], as follows:
\begin{equation}
...
\noindent As denoted in section ?, the parameters of the quadrotor setup are $\Gamma_i(i=1, ..., 8)$ that need to be
identified based on the NRS algorithm. The NLS-TRRLS algorithm is performed in the Matlab R2022b\textregistered. In order to increase accuracy identification of parameters, three scenarios, according to Error! Reference source not found., are considered and performed. When the stopping condition of the NLS algorithm is reached, the best values of the quadrotor parameters are computed, shown in Table 3. Moreover, the intelligent movement of the parameters during the optimization process for finding the true values is shown in Figure 5.
In the first scenario, according to the Figure ?, the quadrotor is able to rotate about only one axis (roll, pitch or yaw axes) to identify $\Gamma_3$ , $\Gamma_6$ and $\Gamma_8$ parameters. In the second scenario,
Figure ? shows $\Gamma_2$ and $\Gamma_5$ parameters are estimated based on the experimental setup, that is free to rotate around its roll and pitch axes. Finally, in the last scenario, according to the Figure
?, $\Gamma_1$ , $\Gamma_4$ and $\Gamma_7$ parameters of the UAV model are identified by rotate the quadrotor setup
...
\begin{table}[H]
	...
	\hline
	\multirow{3}{*}{I} & Roll free & 10 & 10 \\
	& Pitch free & 10 & 10 \\
	& Yaw free & 10 & 10 \\
	\hline
	II & Roll and Pitch free & 10 & 10\\
	\hline
	III & Roll, Pitch, and Yaw free & 10 & 10\\
	\hline
	\end{tabular}
...
\begin{figure}[H]
	...
	\hfil
	% \vspace{-0.25cm}
	\subfloat[]{\includegraphics[width=.45\linewidth]{../Figure/parameter_estimation/yaw/yaw}\includegraphics[width=.425\linewidth]{../Figure/parameter_estimation/yaw/yaw_parameter}
	}
