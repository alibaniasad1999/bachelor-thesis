...
\begin{abstract}
	The accurate attitude control of a quadrotor is necessary, especially when facing disturbance. In this study, a linear quadratic with integral action based on the differential game theory is implemented on a quadrotor experimental setup.
	% In this study, a linear quadratic with integral action based on the differential game theory is implemented on an experimental quadrotor system with three degrees of freedom.
	For this purpose, first, a continuous state-space model of the setup is derived using the linearization of nonlinear equations of motion,
	and its parameters are identified with the experimental results.
	% the experimental results indicate the parameters of the state-space model structure.
...
% The nonlinear dynamics of the quadrotor, as illustrated in Figure 1, will be presented in this section.
...
% These noisy measurements are used by LQIR-DG to control the Euler angles in real-time.
...
\begin{equation}\label{eq:diffeq-end}
	\dot x_6 = (x_2\sin(x_4) + x_3\cos(x_4))/\cos(x_5)
\end{equation}
