...
It is considered that two players are involved in this paper.
...
\begin{equation}\label{system_dynamic}
    ...
    \end{split}
\end{equation}
Where $\boldsymbol{\mathrm{x}}$ is the vector of the state variables, $\boldsymbol{\dot{\mathrm{x}}}$ is the time derivative of the state vector, $\boldsymbol{\mathrm{u_1}}$ is the firts player (controller) input vector, $\boldsymbol{\mathrm{u_2}}$ is the second player (disturbance) input vector, $\boldsymbol{\mathrm{y}}$ is the output vector, $\boldsymbol{\mathrm{A}}$ is the state matrix, $\boldsymbol{\mathrm{B_1}}$ is firts player the input matrix, $\boldsymbol{\mathrm{B_2}}$ is the second player input matrix,
...
Equation (\ref{system_dynamic}) demonstrates how both participants have an impact on the system's dynamics.
...
In this paper we consider the case that players do not cooperate in order to realize their goals.
...
For the system described in equation (\ref{system_dynamic}), LQDG optimum control effort calculates from equation (\ref{LQDG_u}).
...
\end{equation}
In equation \ref{LQDG_u}, $\boldsymbol{\mathrm{k_{i}}}$ is optimal feedback gain. Assuming that the other players will make their worst move, this gain is calculated to minimize the quadratic cost function (equation \ref{cost}) of player number i.
...
, \quad
	...
	\end{bmatrix}
\end{equation}
By introducing a new space state for the system, the remaining design phases of the LQIDG controller are comparable to those of the LQDG controller. LQIDG optimum control effort calculates from equation (\ref{LQIDG_u}).
...
In equation \ref{LQIDG_u}, $\boldsymbol{\mathrm{K_{a_i}}}$ is optimal feedback gain. Assuming that the other players will make their worst move, this gain is calculated to minimize the quadratic cost function (equation \ref{cost_LQIDG}) of player number i.
\begin{equation}\label{cost_LQIDG}
	\begin{split}
		J_i( \boldsymbol{u_1},  \boldsymbol{u_2}) = \int_{0}^{T}\biggl (\boldsymbol{x_a} ^\mathrm{T}(t) \boldsymbol{Q_i} \boldsymbol{x_a}(t)+ 
