...
load("PID_DOBC_d_1")
zero_state = zeros(1, length(DOBC_roll_pitch));
sim_name = "PID_DOBC_d_";
cost = zeros(2, 5);
for i=1:5
    load(append(sim_name, string(i)))
    a1 = timetable(DOBC_roll_pitch(:, 1), 'RowTimes', Time);
    a2 = timetable(DOBC_roll_pitch(:, 2), 'RowTimes', Time);
    a3 = timetable(zero_state', 'RowTimes', Time);
    a4 = timetable(DOBC_roll_pitch(:, 3), 'RowTimes', Time);
    a5 = timetable(DOBC_roll_pitch(:, 4), 'RowTimes', Time);
    a6 = timetable(zero_state', 'RowTimes', Time);
    sim_data = sim("PID_DOBC_finder.slx");
    cost(:, i) = cost_calculator(sim_data, Q_roll, Q_pitch, R, R_d, Time);
end
save('PID_DOBC_cost', 'cost');
